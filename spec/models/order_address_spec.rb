...
RSpec.describe OrderAddress, type: :model do
  describe '購入情報の保存' do
    ...
    context '内容に問題がある場合' do
      ...
      it 'phone_numberが12桁以上だとだと保存できないこと' do
        @order_address.phone_number = '090123456789'
        @order_address.valid?
        expect(@order_address.errors.full_messages).to include('Phone number is too long (maximum is 11 characters)')
      end

      it 'phone_numberが数字以外だとだと保存できないこと' do
        @order_address.phone_number = 'test'
        @order_address.valid?
        expect(@order_address.errors.full_messages).to include('Phone number is invalid. invalid. Input only number')
      end
